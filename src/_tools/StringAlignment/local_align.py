...
def Sub(x: str, y: str) -> float: 
    """
    TODO: Could expand this to return different scores depending on x and y (i.e. if x regularly mistaken for y)
    """
    if x == y:
        return 2
    else:
...
def local_align(X: str, Y: str) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The score matrix generated by local_align
    """    
...
def all_alignment(T: np.ndarray, A: str, B: str) -> list:
    ...
    while np.max(S) > 0:
        ...
        high_vals = [
            (high_vals[0][i], high_vals[1][i], S[high_vals[0][i], high_vals[1][i]]) 
