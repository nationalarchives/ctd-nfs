...
def Sub(x, y) -> float:
    # NOTE: Could expand this to return different scores depending on x and y (i.e. if x regularly mistaken for y)
    if x == y:
        return 2.01
    else:
...
def local_align(X: str, Y: str) -> np.ndarray:
    """    
    ...
    m: int = len(X)
    n: int = len(Y)
    S: np.ndarray = np.zeros((m + 1, n + 1), dtype=float)
...
def one_alignment(S: np.ndarray, X: str, Y: str) -> None:
    """
    ...
    Args:
        S (np.ndarray): The score matrix generated by local_align
        X (str): The first string being aligned
        Y (str): The second string being aligned

    Returns:
        None: The alignment is printed to stdout
    """
...
def all_alignment(T: np.ndarray, A: str, B: str) -> list:
    """
    ...
    Args:
        T (np.ndarray): The score matrix generated by local_align
        A (str): The first string being aligned
        B (str): The second string being aligned

    Returns:
        list: A list of tuples representing the alignment of characters in A and B
    """    
    ...
    """This will end as a list of tuples, each tuple mapping a char in X with one in Y, or None if there is no mapping"""
    alignment = []  
    while np.max(S) > 0:
        i, j = S.shape
        # NOTE: Probably a better way but this finds highest value nearest to bottom right of matrix
